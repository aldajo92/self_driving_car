...
Mode of use
------------
$ rosrun self_driving_car camera _topic:=<image_topic> _camera:=<int> _resolution:=<[height, width]>

_topic: raw image topic path to publish, default 'self_driving_car/camera/image_color/BGR/raw'
...
class CameraPublisher(object):
    ...
    def __init__(self,
                 topic='self_driving_car/camera/image_color/BGR/raw',
                 camindex=0,
                 resolution=(480, 640)):
...
if __name__ == '__main__':
    ...
    topic = rospy.get_param('self_driving_car_camera/topic',
                            'self_driving_car/camera/image_color/BGR/raw')
    camindex = rospy.get_param('self_driving_car_camera/camera', 0)
    resolution = rospy.get_param('self_driving_car_camera/resolution', [240, 320])
    ...
    try:
        ...
        rospy.loginfo('roskey_camera: keyboard interrupt, shutting down')
